...
import { fetchCashFlow, fetchOrder, fetchQuotes } from "@/service";
import type {
  ICashFlowResponse,
...
export function useCashFlowQuery(
  ...
  page: number,
  limit: number,
): UseQueryResult<ICashFlowResponse> {
  const { isLoaded, getToken } = useAuth();

  return useQuery({
    queryKey: ["cashflow", startDate, endDate, page, limit],
    enabled: isLoaded && !!startDate && !!endDate,
    queryFn: async () => {
      const token = await getToken();
      if (!token) {
      ...
      try {
        const res = await fetchCashFlow(token, startDate, endDate, page, limit);
        return res;
      } catch (err) {
